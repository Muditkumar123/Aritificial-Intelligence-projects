...
class Network(nn.Module):
    ...
    def forward(self, state):#state are input entering neural network
        x = F.relu(self.fc1(state)) # X represent hidden neurons which  will active using rectifier function(relu) ,fc1 ke under input state dali h
        q_values = self.fc2(x)  # output layer , q values h ye and softmax use kiya h as default
        return q_values
